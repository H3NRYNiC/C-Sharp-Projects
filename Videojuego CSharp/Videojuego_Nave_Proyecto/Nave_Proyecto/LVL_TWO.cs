...
{
    public partial class LVL_TWO : Form
    {
        ...
        public LVL_TWO()
        {
            ...
            hilo3.Start();
            Thread hiloV1 = new Thread(metodo_AsteroideV1);
            hiloV1.Start();
            Thread hiloV2 = new Thread(metodo_AsteroideV2);
            hiloV2.Start();
            DoubleBuffered = true;
            mov = true;
        }
        ...
        private void LVL_TWO_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.Space:
        ...
        private delegate void asteroideR0(PictureBox p, int x, int y, bool b);
        ...
        private delegate void asteroideV1(PictureBox p, int x, int y, bool b);
        private delegate void asteroideV2(PictureBox p, int x, int y, bool b);
        private delegate void asteroideV3(PictureBox p, int x, int y, bool b);
        ...
        private void delegado_asteroide0(PictureBox p, int x, int y, bool b)
        {
            if (InvokeRequired)
            {
                asteroideR0 asteroide = new asteroideR0(delegado_asteroide0);
        ...
        private void delegado_asteroideV1(PictureBox p, int x, int y, bool b)
        {
            if (InvokeRequired)
            {
                asteroideV1 asteroide = new asteroideV1(delegado_asteroideV1);
                object[] parametros = new object[] { p, x, y, b };
                Invoke(asteroide, parametros);
            }
            else
        ...
        private void delegado_asteroideV2(PictureBox p, int x, int y, bool b)
        {
            if (InvokeRequired)
            {
                asteroideV2 asteroide = new asteroideV2(delegado_asteroideV2);
                object[] parametros = new object[] { p, x, y, b };
                Invoke(asteroide, parametros);
            }
            else
        ...
        private void metodo_AsteroideR()
        {
            ...
            while (mov)
            {
                ...
                while (p.Location.Y >= 0 && p.Location.X >= 0)
                {
                    ...
                    delegado_asteroide0(p, p.Location.X - 5, p.Location.Y, false);
                    velociodad = r.Next(0, 10);
        ...
        private void metodo_AsteroideR1()
        {
            ...
            while (mov)
            {
                ...
                while (p.Location.Y >= 0 && p.Location.X >= 0)
                {
                    ...
                    delegado_asteroide1(p, p.Location.X - 6, p.Location.Y, false);
                    velociodad = r.Next(0, 15);
                    Thread.Sleep(velociodad);
                }
            }
        }
        private void metodo_AsteroideR2()
        {
            ...
            while (mov)
            {
                ...
                while (p.Location.Y >= 0 && p.Location.X >= 0)
                {
                    ...
                    delegado_asteroide2(p, p.Location.X - 5, p.Location.Y, false);
                    velociodad = r.Next(0, 20);
                    Thread.Sleep(velociodad);
                }
            }
        }
        private void metodo_AsteroideV1()
        {
            int velociodad = 0;
            Random r = new Random();
            while (mov)
            {
                ...
                p.SizeMode = PictureBoxSizeMode.Zoom;
                //p.Location = new Point(inicio, y);
                delegado_asteroideV1(p, x, inicio, true);
                while (p.Location.Y >= 0 && p.Location.X >= 0)
                {
                    if (p.Bounds.IntersectsWith(pxNave.Bounds))
                    {
                    ...
                    delegado_asteroideV1(p, p.Location.X - 6, p.Location.Y, false);
                    velociodad = r.Next(0, 25);
                    Thread.Sleep(velociodad);
                }
            }
        }
        private void metodo_AsteroideV2()
        {
            int velociodad = 0;
            Random r = new Random();
            while (mov)
            {
                ...
                p.SizeMode = PictureBoxSizeMode.Zoom;
                //p.Location = new Point(inicio, y);
                delegado_asteroideV2(p, x, inicio, true);
                while (p.Location.Y >= 0 && p.Location.X >= 0)
                {
                    if (p.Bounds.IntersectsWith(pxNave.Bounds))
                    {
                    ...
                    delegado_asteroideV2(p, p.Location.X - 5, p.Location.Y, false);
                    velociodad = r.Next(0, 30);
                    Thread.Sleep(velociodad);
                }
            }
        }
        ...
        private void LVL_TWO_FormClosing(object sender, FormClosingEventArgs e)
        {
            Environment.Exit(0);
        }
    }
